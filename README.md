...
This is a modern Android application built as a technical assignment. It demonstrates a **Master-Detail** flow using the [DummyJSON API](https://dummyjson.com/products), featuring product listing with pagination, detailed views, and a local favorites system.
...
| Partner | App Name | Primary Color | Icon Theme | Target Audience |
| :--- | :--- | :--- | :--- | :--- |
| **Partner A** | Beauty Shop | Rose / Pink | Cosmetics | Skincare & Makeup |
| **Partner B** | Pharmacy | Mint Green | Medical | Medications & Health |

**How to switch:**
Open the **Build Variants** tab in Android Studio and select `partnerADebug` or `partnerBDebug`. Each flavor has its own `resValue` for the app name and custom `colors.xml`.
...
3. **Staged Production Rollout:**
    - **Day 1 (5%):** Monitor Firebase Crashlytics for any flavor-specific regressions.
    - **Day 3 (20%):** Gradual increase if network latency for specific BaseURLs is stable.
    - **Day 5 (50%):** Full monitoring of API consumption limits.
